...
class MpicDcvCheckerService:
    def __init__(self):
        ...
        self.http_client_timeout = (
            os.environ["http_client_timeout"]
            if "http_client_timeout" in os.environ and float(os.environ["http_client_timeout"])
            else 30
        )
        self.dcv_checker = MpicDcvChecker(
            http_client_timeout=self.http_client_timeout, reuse_http_client=True, verify_ssl=self.verify_ssl
        )
